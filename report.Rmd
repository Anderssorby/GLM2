...
for (i in 1:simulations){
  results[results$season==i,]$rank <-  order(-pointtables[[i]]$points,-pointtables[[i]]$goaldiff)
}

ggplot(data=results, aes(results$rank)) + geom_histogram(colour="black",fill="white") + facet_wrap(~ team)

test <- results[results$team==teams[9],]
hist(test$rank)
```
