...
pupper
```
So the $95 \%$ CI of the probability of climbing Mount Everest is $[0.0549, 0.142]$. In another words, one can say with $95 \%$ confidence that between $5.49 \%$ and $14.2 \%$ of those who attempt to climb the mountain succeed in doing so, based on this model.
...
The Pearson $\chi^2$-test is used to test goodness of fit, homogeneity and independece of some data. In this case a test for independence via a contingency table is necessary, as independence between the goals of the home and the away team is questionable. It seems natural that if the away team has more goals the home team would have less due to an intuitive correlation with ball possession, which is a zero sum game.

```{r contingencytable}
ngames <- length(tippeliga[,1])
yhm <- max(tippeliga$yh) + 1
yam <- max(tippeliga$ya) + 1
ctable <- matrix(nrow=yhm, ncol=yam)
dimnames(ctable) = list(c("yh=0","yh=1","yh=2","yh=3","yh=4","yh=5"),c("ya=0","ya=1","ya=2","ya=3","ya=4","ya=5","ya=6"))
for(i in 1:yhm){
  for (j in 1:yam){
    ctable[i,j] <- sum(tippeliga$ya[tippeliga$yh==(i-1)]==(j-1))
  }
}

homerate <- exp(glm(tippeliga$yh ~ 1, family=poisson)$coefficient[1])
awayrate <- exp(glm(tippeliga$ya ~ 1, family=poisson)$coefficient[1])

ptable <- matrix(nrow=yhm, ncol=yam)
for(i in 1:yhm){
  for (j in 1:yam){
    ptable[i,j] <- dpois(i-1,lambda=homerate)*dpois(j-1,lambda=awayrate)
  }
}

etable <- ptable*ngames

chisq <- sum((ctable-etable)^2/etable)
pchisq <- pchisq(chisq,df=30)
pchisq
```

The $\chi^2$ test on 30 degrees of freedom shows that there is no evidence against the assumption of independence. INCLUDE FORMULAS FOR, REVIEW CALCULATION (POSSIBLE PROBLEM: probabilities sum to 0.991...)
...
The preliminary rankings as of the 1st of October is easily determined from the match data.
